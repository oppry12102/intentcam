    ...
    register(
        ToolDef(
            ...
            description = "当你完全理解了图片内容和用户意图后，调这个工具结束识别循环。" +
                ...
                "**action_ids** 字段：**默认应填**，并且按 type 强制映射 — phone → **dial_number**；real_estate_rental → **copy_listing**；recruit_hiring → **save_posting**；id_document → **redact_id**；payment_qr → **scan_to_pay**；location → **open_in_maps**（上述映射的 id 必填，可再叠加 view_details 等通用 id）。把你认为对当前 bubble 有用的 chip id 列出来（系统提示里 actions ∈ {...} 的子集）。" +
