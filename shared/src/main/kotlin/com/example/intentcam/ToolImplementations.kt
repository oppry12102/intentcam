    ...
    register(
        ToolDef(
            ...
            description = "当你完全理解了图片内容和用户意图后，调这个工具结束识别循环。" +
                ...
                "**action_ids** 字段：**默认应填**。列出对当前 bubble 有用的 chip id（actions ∈ {...} 的子集）。" +
                "映射规则：" +
                "识别到电话号码 / 手机号 / 座机 / 400 热线 / 110·119·120·122 → **dial_number**；" +
                "识别到可导航地点 → **open_in_maps**：门牌号、路名、店名（XX店/超市/餐厅/加油站/酒店/专柜/维修店）、" +
                "机构（医院/学校/银行/车站/公司/政府/派出所）、出入口或方向指示（入口/出口/前方/左转/直行）；" +
                "**店名 / 机构名没有街道地址也要带 open_in_maps——导航可以按名称搜索**；" +
                "share 和 open_in_maps **不互斥**：海报 / 告示 / 横幅上署名了具体门店、机构或项目名时" +
                "（如“XX加油站宣”“XX公司承建”“XX专柜前行30米”），share 和 open_in_maps 都填；" +
                ...
                "活动宣传 / 打折信息 等学校、社区、商家张贴的广告。**广告不止海报一种形态，" +
                "以下同样是广告**：带落款（如“XX宣”“XX社区”“XX物业”）的宣传标语、" +
                "便民服务点 / 服务站的招牌（“便民XX点”）、门店玻璃或墙上张贴的服务项目 / 价目列表、" +
                "活动展板 / 拱门 / 横幅（义卖、开盘、开业）、招生报名咨询类的指示牌或海报。" +
                "命中时必须同时在 **ad_markdown** " +
                "输出广告全文转录（markdown，格式同 label_markdown），并在 **ad_bbox** 框出广告主体区域" +
                "（4 角坐标，用于 app 裁剪校正广告图）；广告上的电话 / 地址 / 促销文字同时按上述规则带 " +
                "dial_number / open_in_maps / share，不互斥——广告同时是地点或信息时，" +
                "view_ad 与 open_in_maps / share 要一起填。" +
