...
class LlmClient(@Volatile var config: LlmConfig) {
    ...
    companion object {
         ...
         *  dynamic actions block into [TOOL_USE_SYSTEM] at the
         *  `__ACTIONS_BLOCK__` placeholder.
         *
         *  The placeholder MUST exist verbatim in [TOOL_USE_SYSTEM];
         *  a missing placeholder is a programmer error (silent drift
         *  would let a future prompt edit drop the dynamic block).
         *
         ...
         *
         *  No intent-hint block: the 2026-07-17 architecture retirement
         *  dropped the registered intent taxonomy (and its
         *  type→canonicalAction soft hint). Action selection is the
         *  LLM's job, nudged by the content→action mapping baked into
         *  the `emit_bubble` tool description; content-rescue covers
         *  structured misses (phone / id / payment-QR) add-only.
         *
