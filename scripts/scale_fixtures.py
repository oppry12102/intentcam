...
INTENTS: dict[str, dict] = {
    ...
    "service_institution_20": {
        "seed_gt": "ground_truth_service_institution_20.json",
        # SERVICE_INSTITUTION regex from IntentVerifier.kt (Phase I, v2)
        # v2 dropped 邮政/邮局 (邮编 false-positive) + 工商局/税务局/市场监督
        # (receipt false-positive).
        "regex": re.compile(
            r"医院|卫生院|卫生服务中心|疾控中心|防疫站"
            r"|学校|大学|学院|中学|小学|幼儿园|教育局|培训中心"
            r"|政府|人民政府|街道办事处|居委会|村委会|社区委员会"
            r"|公安局|派出所|法院|检察院|司法局"
            r"|银行|信用社|储蓄所"
            r"|大使馆|领事馆"
        ),
        "type": "service_institution",
        "actions": ["open_in_maps"],
        "categories": [
            (re.compile(r"医院|卫生院|卫生服务中心|疾控中心|防疫站"), "medical_institution"),
            (re.compile(r"学校|大学|学院|中学|小学|幼儿园"), "education_institution"),
            (re.compile(r"公安局|派出所|法院|检察院|司法局"), "government_law_institution"),
            (re.compile(r"政府|人民政府|街道办事处|居委会|村委会"), "government_admin_institution"),
            (re.compile(r"银行|信用社|储蓄所"), "financial_institution"),
            (re.compile(r"大使馆|领事馆"), "diplomatic_institution"),
        ],
...
def scale(name: str, target: int) -> Path:
    ...
    if seed_gt_path.exists():
        seed_gt = json.loads(seed_gt_path.read_text())
        seed_scenes = seed_gt.get("scenes", [])
    else:
        # No pre-existing seed GT — auto-pick all fixtures fresh.
        # Useful when scaling a brand-new intent (Phase I's
        # service_institution had no hand-curated 20-fixture seed).
        seed_gt = {"version": 1, "description": "auto-generated by scale_fixtures.py"}
        seed_scenes = []
